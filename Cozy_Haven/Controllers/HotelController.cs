...
namespace Cozy_Haven.Controllers
{
    ...
    public class HotelController : ControllerBase
    {
        ...
        public HotelController(IWebHostEnvironment environment, ILogger<HotelController> logger, IHotelService hotelService, CozyHavenContext context)
        {
        ...
        [HttpGet("GetHotelsByDestinationId/{destinationId}")]
        public async Task<ActionResult<List<Hotel>>> GetHotelsByDestinationId(int destinationId)
        {
            try
            {
                var hotels = await _hotelService.GetHotelsByDestinationId(destinationId);
                return Ok(hotels);
            }
            catch (NoHotelFoundException)
            {
                return NotFound("No hotels found for the specified destination.");
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult> UpdateHotelDetails(int hotelId, string name, string address, string description)
        {
            try
            {
                _logger.LogInformation($"Updating details for hotel with ID {hotelId}...");
                var updatedHotel = await _hotelService.UpdateHotelDetails(hotelId, name, address, description);
                _logger.LogInformation($"Details updated successfully for hotel with ID {hotelId}.");
                return Ok(updatedHotel);
            }
            catch (NoHotelFoundException ex)
            {
                _logger.LogWarning($"No hotel found with ID {hotelId} for updating details: {ex.Message}");
                return NotFound(ex.Message);
            }
            catch (Exception ex)
            {
                _logger.LogError($"An error occurred while updating details for hotel with ID {hotelId}: {ex.Message}");
                return StatusCode(500, ex.Message);
            }
        }
        [HttpGet("GetHotelsByDestinationName")]
        public async Task<IActionResult> GetHotelsByDestinationName(string destinationName)
        {
            try
            {
                var hotels = await _hotelService.GetHotelsByDestinationName(destinationName);
                return Ok(hotels);
            }
            catch (NoDestinationFoundException)
            {
                return NotFound($"Destination with name '{destinationName}' not found.");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error occurred while retrieving hotels by destination name: {DestinationName}", destinationName);
